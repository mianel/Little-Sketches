...
void ofBoids::setup(int id, float f, float s){
	...
	m_position = ofVec3f(ofRandom(ofGetWidth()/2-100, ofGetWidth()/2 + 100), ofRandom(ofGetHeight()/2-100, ofGetHeight()/2+100));
...
void ofBoids::update(float f, float s, float rA, float rC, float rS){
	...
	float dist = m_position.distance(ofVec2f(ofGetWidth() / 2, ofGetHeight() / 2));
