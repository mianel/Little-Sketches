...
void ofApp::setup(){
    ...
    */
    ...
    gui.add( guiTotalSmall.setup( "Count Childen", 3, 1 , 20 ));
    ...
    gui.add( guiDistance.setup( "Distance", 20, 2, 30 ));
...
void ofApp::draw(){
    if(to_pdf){
        ofBeginSaveScreenAsPDF("render.pdf", true, true);
    }
    ...
    for(int i = 0; i < guiTotal; i++){
        ofPushMatrix();
           ...
           // ofTranslate(0, 0, guiTransZ2 * sinOfTimeX);
